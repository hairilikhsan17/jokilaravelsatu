    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h4>Catat Stok Masuk</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('staff.transaksi.masuk.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                                  id="keterangan" name="keterangan" rows="3">{{ old('keterangan', 'Stok masuk') }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('staff.transaksi.index') }}" class="btn btn-secondary">Batal</a>
                    </div>
                </form>
            </div>
        </div>
